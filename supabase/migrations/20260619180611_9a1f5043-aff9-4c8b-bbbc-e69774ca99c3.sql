...
CREATE TABLE public.ai_settings (
  ...
  system_prompt text, tone text, fallback_message text,
...
CREATE TABLE public.automation_rules (
  ...
  timing_value integer, timing_unit text, message text,
  is_active boolean NOT NULL DEFAULT true,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE public.activity_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  telegram_user_id uuid REFERENCES public.telegram_users(id) ON DELETE SET NULL,
  type text NOT NULL, description text, metadata jsonb,
  created_at timestamptz NOT NULL DEFAULT now()
);
