...
function HistoriasPage() {
  ...
  const list = useQuery({
    queryKey: ["stories"],
    queryFn: async () => (await supabase.from("stories").select("*").order("created_at", { ascending: false })).data ?? [],
  });
  ...
  async function save() {
    ...
    const res = f.id ? await supabase.from("stories").update(payload).eq("id", f.id) : await supabase.from("stories").insert(payload);
